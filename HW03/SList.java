...
/**
 * Created by davidwang on 2/17/15.
 */
/* SList.java */
...
public class SList {
    ...
    public void twin() {
        if (head == null) {
            ...
            if (head.next == null) {
                SListNode temp = new SListNode(head.item);
                head.next = temp;
            } else {
                for (SListNode node = head; (node != null); node = node.next.next) {
                    SListNode temp = new SListNode(node.item, node.next);
                    node.next = temp;
                }
            }
        }
